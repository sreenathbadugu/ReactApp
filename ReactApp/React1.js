...
const ReactDOM = () => {
  ...
  const handleDownloadCertificate = () => {
   ...
   // ctx.fillText(`Congratulations, ${userName}!`, 50, 100);
    //ctx.fillText(`Your score: ${userScore}`, 50, 150);
    ctx.textAlign = 'center';
    ctx.fillText('Certificate of Achievement', canvas.width / 2, 80);
    ctx.font = 'italic 28px Arial';
    ctx.fillText(`This is to certify that ${userName}`, canvas.width / 2, 160);
    ctx.fillText(`has successfully completed the ReactJS Quiz`, canvas.width / 2, 200);
    ctx.font = 'bold 32px Arial';
    ctx.fillText(`Score: ${userScore}`, canvas.width / 2, 250); 
  ...
  return (
    <div style={styles.container}>
      ...
      {quizCompleted && (
        <button onClick={handleDownloadCertificate} style={styles.button}>
